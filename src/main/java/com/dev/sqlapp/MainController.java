...
public class MainController implements Initializable {
    ...
    public void onSelectTable() {
        ...
        // be default we'll show the table contents with predefined query and load in
        // table view
    ...
    public void onRunQuery() {
        try {
            ...
            if (sql.toLowerCase().startsWith("select")) {
                ...
                lblResult
                        .setText("Result: Total " + r + " records, Elapsed Time: " + ((end_time - start_time)) + " ms");
                // close db connection
                resultset.close();
            } else {
                // insert, update, delete query processing
                // in this case we don't have any results in the table, just execute query and
                // show stats
                long start_time = System.currentTimeMillis();
                ...
                lblResult
                        .setText("Result: Affected " + r + " rows, Elapsed Time: " + ((end_time - start_time)) + " ms");
            }
        } catch (SQLException ex) {
            // handle any errors
            ex.printStackTrace();
